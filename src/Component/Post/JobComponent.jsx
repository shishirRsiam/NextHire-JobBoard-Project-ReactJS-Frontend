  ...
  return (
    <div>
      <motion.div
        ...
        <p className="text-gray-600 mb-3">{job.description.slice(0, 250)}.....</p>

        {/* Display categories */}
        <div className="flex flex-wrap gap-2 mb-3">
          {categories && categories.length > 0 ? (
            categories.map((categoryId, index) => (
              <span key={index} className="bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full">{categoryId.name}</span> // Replace categoryId with the actual category name if available
            ))
          ) : null}
        </div>
